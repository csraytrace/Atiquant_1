...
for i, v in enumerate(Pfade):
    ...
    for j in All_asr_files(v):
        Calc = Calc_I(Messzeit = a, Röhrenstrom=b, Emax = c * volt_proz + volt, charzucont = czuc, Element_Probe=j[0], activeLayer=layer, Kontaktmaterialdicke=kontakt)
        Ele = Element(Element=j[0])
        alle_geo.append((j[1]/Calc.Intensität_K(), Ele.K_gemittel_ubergang(), Ele.Get_Elementsymbol()))
        print(j,Ele.Get_Elementsymbol(), "K")
    for L in All_asr_files(Pfade_L[i]):
        a, b, c = Röhreneinstellung_L[i]
        Calc = Calc_I(Messzeit = a, Röhrenstrom=b, Emax = c * volt_proz + volt, charzucont = czuc, Element_Probe=L[0], activeLayer=layer, Kontaktmaterialdicke=kontakt)
        Ele = Element(Element=L[0])
        alle_geo.append((L[1]/Calc.Intensität_L(), Ele.L_gemittel_ubergang(), Ele.Get_Elementsymbol()+"_L"))
        print(L,Ele.Get_Elementsymbol(), "L")
...
with open(path, 'a') as file:
    ...
    file.writelines("\n")
    textkopie += ("Geometriefaktor mit Spannungsänderung:"+str(volt)+"\n") +(str(alle_geo)) +("\n")
#with open(path, 'a') as file:
    file.writelines(textkopie)
print(alle_geo)
