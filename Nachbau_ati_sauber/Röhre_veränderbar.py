...
class Röhre_veränderbar():
    ...
    def Röhrenspektrum_cont_berechnen(self):
        #Anoden_Tau = self.Daten_Anode.Massenabsorptionskoeffizient_array()
        Anoden_Tau = self.Daten_Anode.tau

        #Fenster_Mü = self.Daten_Fenster.Massenschwächungskoeffizient_array()
        ...
        for i, E in enumerate(Anoden_Tau[0]):
            ...
            Photonenanzahl = sigma * fx * self.Röhrenstrom * self.Messzeit * self.step * self.Raumwinkel #* 1/self.step  #1/step damit keine bins entstehen
            retlist.append(Photonenanzahl)
            nach_Fenster.append(Photonenanzahl * BeerLambert(Fenster_Mü[1][i], Dichte_Fenster, self.Fensterdicke, self.Fenstereinfallwinkel))
