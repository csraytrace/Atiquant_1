...
counter = [0]
ax_save = plt.axes([0.81, 0.925, 0.1, 0.045])
button = Button(ax_save, 'Scharf speichern')

def save_plot(event):
    if counter[0]==0:
        fig.savefig("C:\\Users\\julia\\OneDrive\\Dokumente\\A_Christian\\Masterarbeit\\Masterarbeit\\Prozent_Anregung_genau.png", dpi=600, bbox_inches='tight')
        print("Plot scharf gespeichert!")
    else:
        fig.savefig("C:\\Users\\julia\\OneDrive\\Dokumente\\A_Christian\\Masterarbeit\\Masterarbeit\\Prozent_Anregung2_genau.png", dpi=600, bbox_inches='tight')
        print("Plot scharf gespeichert!")
    counter[0] += 1

button.on_clicked(save_plot)

Ki=Calc_I(Konzentration=[1], P1=["Pb"], Übergänge=[0], Emax=40)
Ele=Element(Element="Pb")

x = np.empty(540)
x[0] = 13.05  # Dein spezieller Startwert!
x[1:] = 13.05 + 0.05 * np.arange(1, 540)
...
x1_init=Tube.Countrate_gesamt[0][index]
vline1 = ax.axvline(x=x1_init, color='r', linestyle='--', label=f'x1 = {x1_init:.2f}')
vline2 = ax.axvline(x=x2_init, color='b', linestyle='--', label=f'x2 = {x2_init:.2f}')
...
print("??",Tube.Countrate_gesamt[0][index])

# Slider für x1
ax_slider1 = plt.axes([0.1, 0.1, 0.8, 0.03], facecolor='lightgoldenrodyellow')
slider1 = Slider(ax_slider1, 'x1', x[0], Emax, valinit=x1_init, valstep=step)

# Slider für x2
ax_slider2 = plt.axes([0.1, 0.05, 0.8, 0.03], facecolor='lightgoldenrodyellow')
slider2 = Slider(ax_slider2, 'x2', x[0], Emax, valinit=x2_init, valstep=step)
...
def update(val):
    ...
    Summe = y.sum()
    Summe2 = y[int(x1/step - x[0]/step):int(x2/step - x[0]/step)].sum()
    #print(int(x1/step  - x[0]/step),int(x2/step - x[0]/step))
